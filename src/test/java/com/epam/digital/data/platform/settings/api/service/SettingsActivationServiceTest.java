...
class SettingsActivationServiceTest {
  ...
  void expectDeactivateNotPassUserRoleVerification() {
    ...
    SettingsDeactivateChannelInputDto inputDto = new SettingsDeactivateChannelInputDto();
    Assertions.assertThatThrownBy(
            () -> settingsActivationService.deactivateChannel(Channel.DIIA, inputDto, "token"))
        .isInstanceOf(AccessDeniedException.class)
        .hasMessage("Invalid user role for deactivate operation");
    verify(auditFacade)
        .sendDeactivationAuditOnFailure(Channel.DIIA, inputDto, "User role verification failed");
  }
}
