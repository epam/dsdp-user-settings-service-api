...
public interface NotificationChannelRepository extends CrudRepository<NotificationChannel, UUID> {
  ...
  @Modifying
  @Query("DELETE FROM notification_channel WHERE settings_id=:settingsId AND channel=CAST(:channel as channel_enum)")
  void deleteBySettingsIdAndChannel(
      @Param("settingsId") UUID settingsId,
      @Param("channel") Channel channel);

  @Modifying
  @Query("DELETE FROM notification_channel WHERE settings_id=:settingsId")
  void deleteBySettingsId(@Param("settingsId") UUID settingsId);
}
