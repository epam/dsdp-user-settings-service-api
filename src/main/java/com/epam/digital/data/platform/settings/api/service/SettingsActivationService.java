...
public class SettingsActivationService {
  ...
  private static final String USE_ROLE_ERROR_MASSAGE = "User role verification failed";
  private static final String COMMUNICATION_ERROR_MASSAGE = "Communication channel verification failed";
  ...
  public void activateChannel(ActivateChannelInputDto input, Channel channel, String accessToken) {
    if (!userRoleVerifierService.verify(channel, accessToken)) {
      auditFacade.sendActivationAuditOnFailure(channel, input, USE_ROLE_ERROR_MASSAGE);
    ...
    if (!successfullyVerified) {
      auditFacade.sendActivationAuditOnFailure(
          channel, input, COMMUNICATION_ERROR_MASSAGE);
      throw new ChannelVerificationException(COMMUNICATION_ERROR_MASSAGE);
    }
  ...
  public void deactivateChannel(
    ...
    if (!userRoleVerifierService.verify(channel, accessToken)) {
      auditFacade.sendDeactivationAuditOnFailure(channel, input, USE_ROLE_ERROR_MASSAGE);
